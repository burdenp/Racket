...
(define-type IO
  ...
  [SetRef   VAL VAL]) ; ref, new value) ; IO IO
...
;;creates a new ref with x inside
(: ref : Any -> REF)
(define (ref x) (Ref (box x)))
;;opens a ref r and returns its contents
(: unref : REF -> Any)
(define (unref r) (cases r [(Ref b) (unbox b)]))
;;sets a given ref to a given x
  ...
  (let* ([val (strict val)]
    ...
    (if (not io)
      ...
      (cases io
        ...
        [(SetRef x y) (execture-setref x y)]))))
