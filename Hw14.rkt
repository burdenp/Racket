...
(define (runner env)
  ...
  (let ([comp-fval compile]
           [comp-arg compile]
           [comp-body compile-body]
           [comp-box compile-get-boxes])
  (cases expr
    ...
    [(Call fun-expr arg-exprs)
     ...
     (lambda ([env : ENV])
     (let ([fval ((comp-fval fun-expr) env)]
           ;; delay evaluating the arguments
           
           [arg-vals 
            (let ([final-exprs (map comp-arg arg-exprs)])
              (lambda () (map (runner env) final-exprs)))])
       (cases fval
         [(PrimV proc) (proc (arg-vals))]
         [(FunV names body fun-env byref?)
          (comp-body body (if byref?
                            (raw-extend names
                                        ((comp-box arg-exprs) env)
                                        fun-env)
                            (extend names (arg-vals) fun-env)))]
         [else (error 'compile "function call with a non-function: ~s"
                      fval)])))]
    [(If cond-expr then-expr else-expr)
     (let ([comp-cond compile])
     (lambda ([env : ENV])
     ((comp-cond (if (cases ((comp-cond cond-expr) env)
