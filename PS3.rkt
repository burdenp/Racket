...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(list 'sqrt muwae) (Sqrt (parse-sexpr muwae))]
...
(define (subst expr from to)
  (cases expr
    ...
    [(Sqrt muwae) (Sqrt (subst muwae from to))]
...
(define (sqrt+ ns)
  (cond [(null? ns) '()]
        [(< (first ns) 0) (error 'neg-sqrt "`sqrt' requires a non-negative input")]
...
(test (run "{with with {x 5} {/ x 5}}") =error> 
      "parse-sexpr: bad `with' syntax in (with with (x 5) (/ x 5))")
(test (run "{with {x 5} y}") =error> "free identifier")
(test (run "{cons {with }}") =error> "parse-sexpr: bad syntax in (cons (with))")
(test (run "{sqrt 9}") => '(3 -3))
(test (run "{sqrt 1}") => '(1 -1))
(test (run "{sqrt 0}") => '(0 0))
