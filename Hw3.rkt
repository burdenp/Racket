...
;; PostfixItem used for POST
(define-type PostfixItem = (U PUWAE '+ '- '* '/))
(: parse-sexpr : Sexpr -> PUWAE)
;; to convert s-expressions into PUWAEs
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(cons 'post more) (Post (parse-post-item more))]
...
(define (subst expr from to)
  (: post-subst : PostfixItem -> PUWAE)
  (define (post-subst item)
    (if (symbol? item)
      (if (eq? item from) to expr)
      item))
  (cases expr
    [(Num n) expr]
    ...
    [(Post more) (Post (post-subst more))]))
...
(define (eval expr)
  (cases expr
    ...
    [(Post more) (post-eval (first more) (rest more))]))

(: post-eval : PostfixItem PostfixItem -> Number)
...
(test (run "{with {x } 2}") =error> "bad `with' syntax in (with (x) 2)")
(test (run "{+ 2}") =error> "bad syntax in (+ 2)")
(test (run "{with {x {/ 5 5}} {* x x}}") => 1)